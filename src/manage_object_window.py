...
class ManageObjectWindow(QMainWindow):
    ...
    def __init__(self, obj: QListWidgetItem, conn: multiprocessing.connection.Connection):
        ...
        scaling_value_label = QLabel('Valor:')
        self.scaling_value_input = QTextEdit('')
        ...
        third_row_layout.addWidget(scaling_value_label, 1, 0)
        third_row_layout.addWidget(self.scaling_value_input, 1, 1)
        third_row_layout.addWidget(scaling_button, 2, 0, 1, 2)
    ...
    def scale_obj(self):
        scale_value = int(self.scaling_value_input.toPlainText())
        print(scale_value)
        print(self.__object)
        self.__conn.send((Event.DRAWABLE_SCALED, self.__object.data(1)))

    def translate_obj(self):
        translate_x_value = int(self.translation_x_input.toPlainText())
        translate_y_value = int(self.translation_y_input.toPlainText())
